# AGENTS.md -- Molt: Setup & Onboarding Guide for Coding Agents

You're installing **Molt** for the user. Your job is not just to set it up -- it's to get them excited about using it.
...
> **Molt** replaces your new tab page with a clean dashboard of everything you have open, grouped by domain.
>
...
>
> You should see "Molt" appear in your extensions list.
...
> You're all set! Open a **new tab** and you'll see Molt.
>
> Here's how it works:
...
- Molt is a pure Chrome extension. No server, no Node.js, no npm.
- Saved tabs are stored in `chrome.storage.local` (persists across sessions).
- 100% local. No data is sent to any external service.
- To update: `cd molt && git pull`, then reload the extension in `chrome://extensions`.
